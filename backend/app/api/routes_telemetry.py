    ...
    for ev in items:
        ...
        summaries.append(
            SecurityEventSummary(
                ...
                ml_classification_performed=ev.ml_classification_performed,
                predicted_family=ev.predicted_family,
                class_confidence=round(ev.class_confidence, 4) if ev.class_confidence is not None else None,
                normalized_anomaly_score=round(ev.normalized_anomaly_score, 4) if ev.normalized_anomaly_score is not None else None,
...
async def get_security_event_detail(
    ...
    return SecurityEventDetail(
        ...
        raw_decision_score=dec.raw_decision_score if dec else None,
        is_statistical_anomaly=ev.is_statistical_anomaly,
        risk_score=ev.risk_score,
        severity=ev.severity,
        triage_status=ev.triage_status,
        explanation=dec.explanation if dec else None,
        supervised_model_key=dec.supervised_model_key if dec else None,
        anomaly_model_key=dec.anomaly_model_key if dec else None,
