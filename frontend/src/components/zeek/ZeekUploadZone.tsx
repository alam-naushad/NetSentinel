...
export const ZeekUploadZone: React.FC<ZeekUploadZoneProps> = ({
  onFileSelected,
  isAnalyzing,
}) => {
  ...
  const validateAndSelect = (file: File) => {
    ...
    const isValidExt = ext.endsWith('.log') || ext.endsWith('.json') || ext.endsWith('.tsv');
    if (!isValidExt) {
      setValidationError('Please select a valid Zeek log file (.log, .json, or .tsv).');
      return;
    }
            ...
            {selectedFile ? (
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  {formatBytes(selectedFile.size)} • Ready for telemetry processing
                </p>
              </div>
            ) : (
              <div className="space-y-1">
                <p className="text-sm font-medium text-slate-200">
                  <span className="text-blue-400 font-semibold">Click to upload</span> or drag and drop Zeek log
                </p>
                <p className="text-xs text-slate-400">
                  Supports <code className="text-slate-300">.log</code>, <code className="text-slate-300">.json</code>, and <code className="text-slate-300">.tsv</code> formats (up to 50 MB)
                </p>
              </div>
            )}
          </div>
